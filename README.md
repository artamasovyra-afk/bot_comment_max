...
- мини-приложение показывает ленту в формате чата, автоматически открывается на последнем непрочитанном или последнем комментарии, подтягивает новые комментарии, умеет подгружать ранние сообщения, отвечать на комментарии, прикладывать фото, показывает изображения как компактные превью с открытием полного размера по короткому тапу или клику, открывает меню действий по долгому тапу на мобильном или по правому клику на десктопе, позволяет пользователю удалить или отредактировать свой комментарий и даёт администраторам модерировать комментарии только в доступных им каналах
...
13. Бэкенд валидирует подпись `initData`, сохраняет комментарий и, если у канала есть legacy-чат обсуждения, дублирует комментарий туда.
