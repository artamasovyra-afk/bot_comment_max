...
- мини-приложение показывает ленту в формате чата, автоматически открывается на последнем непрочитанном или последнем комментарии, подтягивает новые комментарии, умеет подгружать ранние сообщения, отвечать на комментарии, прикладывать фото, показывает изображения как компактные превью с открытием полного размера по короткому тапу или клику, открывает меню действий по долгому тапу на мобильном или по правому клику на десктопе, делает имя и аватар автора кликабельными с открытием профиля пользователя и даёт администраторам модерировать комментарии только в доступных им каналах
...
- тап или клик по имени автора открывает профиль пользователя
- тап или клик по аватару автора открывает профиль пользователя
- если внешний `profileUrl` MAX недоступен, WebApp показывает внутренний профиль с именем, username, количеством комментариев в текущем канале, датой первого комментария и последними сообщениями автора
...
13. Имя и аватар автора комментария открывают профиль пользователя; если внешняя ссылка MAX отсутствует, показывается внутренний профиль WebApp в рамках текущего канала.
14. Бэкенд валидирует подпись `initData`, сохраняет комментарий и, если у канала есть legacy-чат обсуждения, дублирует комментарий туда.

## API мини-приложения

- `GET /api/healthz` — возвращает `ok`, текущую `version` и `delivery_mode`
- `GET /api/users/<user_id>/profile?post_ref=<post_ref>` — возвращает безопасный профиль автора для текущего канала: имя, username, количество комментариев, дату первого комментария и недавние комментарии без приватных полей
