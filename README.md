...
Версия для демонстрации, тестового стенда для прода рекомендуется стек Go + PostgreSQL + TypeScript
...
- при добавлении кнопки бот не меняет текст самого поста и не дописывает служебную подпись
...
При этом бот обновляет только `attachments` сообщения, чтобы исходный текст поста оставался нетронутым и администраторы могли продолжать редактировать пост в интерфейсе MAX.
Пересланные посты из других каналов по текущему бизнес-правилу игнорируются: бот не создаёт для них обсуждение и не добавляет кнопку `Комментарии`.
...
В списке `/super-admin → Администраторы` для записей администратора показываются `MAX user id`, имя, фамилия, `display_name` и `username`. Если профиль ещё не сохранён в локальной базе, панель пытается один раз подтянуть его из MAX по назначенному каналу администратора и затем кэширует в `admin_users`.
...
7. После одобрения кнопка `Комментарии` будет автоматически появляться у новых обычных постов канала.

Если канал уже был подключён к сервису другим администратором, доступ второму администратору тоже выдаётся через отдельную заявку супер-администратору:

1. Откройте личный чат с ботом и примите условия использования, если ещё не делали этого.
2. Перешлите боту любой пост из уже подключённого канала.
3. Если условия ещё не приняты, бот покажет текст согласия и попросит принять условия, а затем переслать пост повторно.
4. После повторной пересылки бот проверит, что канал уже активен и что вы администратор этого канала в MAX.
5. Бот создаст заявку типа `add_channel_admin` для супер-администратора.
6. Только после одобрения заявки канал появится у вас в `/admin`.

Один подключённый канал может иметь нескольких `channel_admin`. В `/super-admin` у канала отображается список его активных администраторов.

Супер-администратор в разделе `Заявки` видит два типа заявок:

- `Подключение канала` для нового канала
- `Добавление администратора к каналу` для уже подключённого канала
...
6. При этом бот не дописывает текст в пост, а обновляет только inline-кнопку и вложения сообщения.
7. Кнопка открывает ссылку вида `https://max.ru/<botName>?startapp=post_<token>`.
8. MAX открывает мини-приложение, а `window.WebApp.initDataUnsafe.start_param` содержит ссылку на конкретный пост.
9. Мини-приложение запрашивает у бэкенда только этот пост и только его комментарии.
10. При отправке комментария мини-приложение передаёт `window.WebApp.initData`.
11. При открытии WebApp экран автоматически прокручивается к последнему непрочитанному комментарию, а если непрочитанных нет — к последнему комментарию.
12. Состояние прочтения хранится отдельно для каждого пользователя и поста.
13. Изображения в комментариях отображаются как компактные превью; при клике открывается полноэкранный просмотрщик, а несколько изображений показываются сеткой с навигацией. Старые media URL автоматически нормализуются к текущему домену WebApp.
14. Если для автора когда-либо появится подтверждённый `profileUrl` MAX, имя и аватар смогут открыть профиль через MAX Bridge; без такого URL они остаются некликабельными.
15. Комментарии поддерживают реакции эмодзи `👍 ❤️ 😂 🔥 😮 😢`: выбор реакции открывается через меню действий, а под комментарием отображаются только уже поставленные счётчики.
16. В комментариях-ответах тап или клик по блоку цитаты прокручивает список к исходному сообщению и кратко подсвечивает его.
17. Бэкенд валидирует подпись `initData`, сохраняет комментарий и, если у канала есть legacy-чат обсуждения, дублирует комментарий туда.
