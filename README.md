...
То же самое работает и для новых постов, которые были пересланы в подключённый канал из другого канала: для кнопки `Комментарии` бот ориентируется на канал публикации, а не на исходный forwarded-источник.
Если MAX не разрешает напрямую отредактировать forwarded-wrapper пост, бот публикует рядом отдельное reply-сообщение с кнопкой `Комментарии`, чтобы обсуждение всё равно было доступно.
...
7. После одобрения кнопка `Комментарии` будет автоматически появляться у новых постов канала, включая пересланные посты из других каналов.
